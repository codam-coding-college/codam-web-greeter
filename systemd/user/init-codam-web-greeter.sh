...
if [ -z "$WALLPAPER" ]; then
	/usr/bin/echo "No Gnome screensaver wallpaper found" | /usr/bin/systemd-cat -p warning
	/usr/bin/exit 1
fi

/usr/bin/echo "Found Gnome screensaver wallpaper $WALLPAPER" | /usr/bin/systemd-cat -p info
...
if [[ "$WALLPAPER" == file://* ]]; then
	...
	if [ -f "$WALLPAPER_PATH" ]; then
		...
		/usr/bin/echo "Copied wallpaper $WALLPAPER_PATH to $TMP_WALLPAPER_PATH" | /usr/bin/systemd-cat -p info
	else
		/usr/bin/echo "Wallpaper $WALLPAPER_PATH does not exist" | /usr/bin/systemd-cat -p warning
	fi
else
	/usr/bin/echo "Wallpaper $WALLPAPER is not a local file" | /usr/bin/systemd-cat -p warning
fi
