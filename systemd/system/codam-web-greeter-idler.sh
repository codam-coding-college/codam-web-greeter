...
while IFS= read -r line; do
	...
	/usr/bin/echo "Checking idletime for user $USERNAME on display $DISPLAY"
	...
	if [ "$IDLE_TIME" -gt 2520000 ]; then
		/usr/bin/echo "Session for user $USERNAME has been idle for over 42 minutes, forcing logout now"
